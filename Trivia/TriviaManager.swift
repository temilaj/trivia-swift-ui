...
class TriviaManager: ObservableObject {
    ...
    private(set) var trivia: [APIResponse.Result] = []
    @Published private(set) var length = 0

    // Variables to set question and answers
    @Published private(set) var index = 0
    @Published private(set) var question: AttributedString = ""
    @Published private(set) var answerChoices: [Option] = []
//
...
//    // Call the fetchTrivia function on intialize of the class, asynchronously
    ...
    func setQuestion() {
        ...
        if index < length {
            ...
            answerChoices = currentTriviaQuestion.options
        }
    }

    // Function to know that user selected an answer, and update the score
    func selectAnswer(option: Option) {
        answerSelected = true
